...
impl OpenAICompatibleClient {
    ...
    pub async fn chat_completion(
        ...
        debug!("Sending {} chat completion request to model: {}", self.provider_name, model);
        ...
        if self.provider_name == "openrouter" {
            req = req.header("HTTP-Referer", "https://github.com/binoymanoj/jarvis")
        ...
        if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
            warn!("{} 429 Too Many Requests on model {model}", self.provider_name);
        ...
        let choice = val.get("choices")
            .and_then(|c| c.get(0))
            .and_then(|c| c.get("message"))
            .ok_or_else(|| JarvisError::Other(format!("Invalid response format from {}", self.provider_name)))?;

        let text_content = choice.get("content").and_then(|c| c.as_str()).map(|s| s.trim().to_string());

        let mut tool_calls = Vec::new();
        if let Some(calls) = choice.get("tool_calls").and_then(|tc| tc.as_array()) {
            for (idx, call) in calls.iter().enumerate() {
                let id = call.get("id").and_then(|i| i.as_str()).unwrap_or(&format!("call_{idx}")).to_string();
                let fn_obj = call.get("function");
                let name = fn_obj.and_then(|f| f.get("name")).and_then(|n| n.as_str()).unwrap_or_default().to_string();
                ...
                tool_calls.push(AIToolCall { id, name, args: args_val });
            }
        }

        Ok(AIResponse { text: text_content, tool_calls })
    }
}
...
impl AnthropicClient {
    ...
    pub async fn create_message(
        ...
        if let Some(content_arr) = val.get("content").and_then(|c| c.as_array()) {
            for (idx, item) in content_arr.iter().enumerate() {
                ...
                if item_type == "text" {
                    ...
                    let id = item.get("id").and_then(|i| i.as_str()).unwrap_or(&format!("call_{idx}")).to_string();
        ...
        Ok(AIResponse { text: combined_text, tool_calls })
    }
}
...
impl AIClient {
    pub fn from_settings(settings: &Settings) -> Result<Self> {
        ...
        match provider.as_str() {
            "openai" => {
                let key = settings.openai_api_key.as_deref()
                    .ok_or_else(|| JarvisError::EnvVarMissing("OPENAI_API_KEY".to_string()))?;
                Ok(AIClient::OpenAI(OpenAICompatibleClient::new("openai", key, "https://api.openai.com/v1")))
            }
            "anthropic" | "claude" => {
                let key = settings.anthropic_api_key.as_deref()
                    .ok_or_else(|| JarvisError::EnvVarMissing("ANTHROPIC_API_KEY".to_string()))?;
                Ok(AIClient::Anthropic(AnthropicClient::new(key)))
            }
            "groq" => {
                let key = settings.groq_api_key.as_deref()
                    .ok_or_else(|| JarvisError::EnvVarMissing("GROQ_API_KEY".to_string()))?;
                Ok(AIClient::OpenAI(OpenAICompatibleClient::new("groq", key, "https://api.groq.com/openai/v1")))
            }
            "openrouter" => {
                let key = settings.openrouter_api_key.as_deref()
                    .ok_or_else(|| JarvisError::EnvVarMissing("OPENROUTER_API_KEY".to_string()))?;
                Ok(AIClient::OpenAI(OpenAICompatibleClient::new("openrouter", key, "https://openrouter.ai/api/v1")))
            }
            _ => {
                // Default to Gemini
                let key = settings.gemini_api_key.as_deref()
